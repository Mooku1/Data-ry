$(document).on('ready page:load', function() {
	...
	// ------ Previous Two Weeks Graph ------ //


			twoWeekGraph = function(){

				//Set an empty for where our graph data point objects will go
				var twoWeekData = [];

				//Most recent date variable
				var mostRecentDate = convertDate(data[data.length-1].date);

				//Loop through all of our dates, return the ones that occurred with two
				//weeks of our most recent date and push them to the twoWeekData array
				for (i = 0; i < data.length; ++i){
					if (convertDate(data[i].date) > (mostRecentDate - (14 * dayToMs))){
				...
				var w = document.getElementById("twoWeekGraph").offsetWidth;
				var h = document.getElementById("twoWeekGraph").offsetHeight;
				var padding = 5;
				var leftPadding = 20;
				...
				var xScale = d3.time.scale()
							.domain([convertDate(twoWeekData[0].date), d3.time.day.offset(lastDate, 1)])
				...
				var xAxis = d3.svg.axis()
							...
							.tickValues(countTicks(twoWeekData, Math.ceil(twoWeekData.length/10))) //call the ticks function to create ticks
							.tickSize(0)
							.tickPadding(8);
			...
			// ------ Draw the SVG element ------ //
				...
				var svg = d3.select("#twoWeekGraph")
							.append("svg")
							.attr("width", w)
							.attr("height", h);
